...
//        Players.all().stream().filter(this::isPlayerKiller).
        ...
        if((Players.all().stream().anyMatch(this::isPlayerKiller)))
                return false;
        if(Config.INSTANCE.getTrainingArea().contains(getMyTile()) && Players.all().size() >= 3)
            return false;
        return (Config.INSTANCE.getTrainingArea().contains(getMyTile()) || !getNearbyDragons().isEmpty() &&
                Players.all().stream().noneMatch(p -> getLevelDifference(p) && isPlayerKiller(p)));
    }
}
